...
-- set orc_enable_filter_pushdown=off;
 ...
 set new_executor_enable_self_tuning = on;
 set min_datasize_to_combine_segment=2048; -- 1..2048 MB
...
begin
    ...
    raise notice '% %.%', oushudb_version_number, oushudb_version_number_major, oushudb_version_number_minor;
   if oushudb_version_number_major >=5 or oushudb_version_number_major >= 4 and oushudb_version_number_minor >= 4 then
...
:sql_prefix /*Q20*/ select s_name, s_address from supplier, nation where s_suppkey in ( select ps_suppkey from partsupp where ps_partkey in ( select p_partkey from part where p_name like 'forest%' ) and ps_availqty > ( select 0.5 * sum(l_quantity) from lineitem where l_partkey = ps_partkey and l_suppkey = ps_suppkey and l_shipdate >= date '1994-01-01' and l_shipdate < date '1995-01-01' ) ) and s_nationkey = n_nationkey and n_name = 'CANADA' order by s_name limit 100;
